...
const Navbar = ({ toggleDarkMode, darkMode }) => {
  return (
    <nav className={`fixed left-0 top-0 h-full w-72 py-10 px-6 flex flex-col items-center space-y-12 
      ...
      <h2 className="text-3xl font-thin">My Portfolio</h2>

      <button onClick={toggleDarkMode} 
        className="px-5 py-2 border border-green-400 rounded-md text-green-400 
          hover:bg-green-400 hover:text-black transition">
        {darkMode ? "Light Mode" : "Dark Mode"}
      </button>

      <ul className="flex flex-col space-y-8 text-lg font-light tracking-wide">
