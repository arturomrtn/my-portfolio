...
const Navbar = ({ toggleDarkMode, darkMode, toggleNavbar, isMobile }) => {
  ...
  return (
    <nav className={`fixed left-0 top-0 h-full ${isMobile ? "w-full" : "w-72"} py-10 px-6 flex flex-col items-center space-y-12 transition-all ${darkMode ? "bg-black text-green-400" : "bg-white text-black"} z-50`}>
      ...
      <ul className="flex flex-col text-lg space-y-2">
        {["Home", "About", "Projects", "Contact"].map((item) => (
          <li key={item}>
            <a href={`#${item.toLowerCase()}`} className="hover:text-green-400 transition" onClick={toggleNavbar}>
              {item}
            </a>
          </li>
        ))}
